...
const Sidebar = () => {
  ...
  return (
    <motion.section
      ...
      variants={variants}
      className={'w-1/5 bg-mainBg border-r-2 border-bordColor h-screen flex flex-col justify-between items-center gap-10 relative '
      ...
      <div className='flex flex-col justify-content items-center gap-8'>
        ...
        <div id='navlinks-box' className='flex flex-col justify-center items-start gap-4 w-full mt-4'>
          {navItems.map((item, index) => (
            <NavLink
              ...
              activeClassName="bg-actNav shadow-xl font-bold text-actText"
            >
      ...
      <div id='logout-box' className='w-full flex flex-col justify-start items-center gap-3 cursor-pointer'>
        <div className='bg-mainBorder w-full h-[1px]'></div>
        <div className='flex justify-center items-center gap-2'>
          <MdLogout className='text-darkText h-6 w-6' />
          <span className={'text-darkText text-lg ' + (isExpanded ? 'flex' : 'hidden')}>Logout</span>
        </div>
      </div>
