...
const Inbox = () => {
    ...
    const options = ['Dan Edward Manuel', 'Justin Marucut', 'Dan Edward Manuel', 'Justin Marucut', 'Dan Edward Manuel', 'Justin Marucut', 'Dan Edward Manuel', 'Justin Marucut', 
    'Dan Edward Manuel', 'Justin Marucut', 'Dan Edward Manuel'];
    ...
    const [data, setData] = useState([
      {
        ...
        message: '1 Bedroom unit with Balcony facing Amenities 1 Bedroom unit with Balcony facing Amenities',
        date: 'April 18, 2024'
      },
      {
        id: 2,
    ...
    const customStyles = {
      ...
      cells: {
        style: {
          ...
          a: { // targeting link elements specifically
    ...
    <>
      <section className='w-4/5 grow bg-backColor h-screen overflow-y-auto flex flex-col justify-start items-center gap-2 p-4 scrollbar-thin scrollbar-webkit'>
        <div id='main-section' className='grid lg:grid-cols-3 grid-cols-1 gap-4 w-full h-screen'>
          <div id='left' className='col-span-2 p-3 gap-3 flex flex-col items-center h-screen'>
            <div id='leftContainer' className='flex flex-col bg-mainCol border-b-[1px] border-mainBorder p-4 w-full h-full rounded-xl justify-between items-center shadow'>
              ...
              <div id='tableContainer' className='flex flex-col flex-grow p-2 mt-1 overflow-hidden'>
          ...
          <div id='right' className='p-2 flex flex-col justify-center items-center gap-4 h-screen'>
            <div className='bg-mainCol border-b-[1px] border-mainBorder p-4 w-full h-screen rounded-xl flex-col justify-center items-center gap-6 shadow'>   
                <div className='w-full flex justify-between items-center'>
                    <h1 className='text-2xl text-act-text font-semibold mb-2'>Send a Message</h1>
                </div>
                <h1 className='text-base text-darkText font-semibold mb-1' >Name:</h1>
                <div className="dropdown relative">
                    <div className=" bg-white text-darkText text-mg flex font-bold p-2 items-center justify-between shadow rounded-xl"
                    ...
                    {isActive && (
                        <div className="dropdownContent text-darkText w-full p-2 border-b-[1px] border-mainBorder bg-white mt-1 rounded-xl absolute max-h-48 overflow-y-auto scrollbar-thin scrollbar-webkit">
                            {options.map(option => (
                                <div key={option} onClick={() => handleOptionSelect(option)} className="dropdownItem mt-1 hover:bg-actNav cursor-pointer">{option}</div>
                            ))}
                        </div>
                    )}
                </div>
                  <textarea id='message'
                    ...
                    style={{width: "100%", height: "420px", wordWrap: "break-word"}}
                    value={message} onChange={(e) => setMessage(e.target.value)} />
                  <div className="w-full flex flex-col sm:flex-row items-end justify-end gap-2">
                    <button id='sendMessage'
                      className='mt-2 justify-end items-end bg-notActText hover:bg-cirlce text-white font-bold py-2 px-4 rounded-full'>Send
                    </button>
                  </div>
            </div>
          </div>
